...
const TEAM_CODE_ERROR =
  "That team code isn't right. Ask your manager for the current one.";
const INVITE_ERROR =
  "Your invite from 3C is no longer open. Ask your manager for a new invite or the team code.";
    ...
    <ol className={a.steps} aria-label="How sign-up works">
      {SIGNUP_STEPS.map((step) => (
        <li key={step.n}>
          <b>{step.n}</b>
          <span>
            {invited && step.n === 1 ? "Use your invite from 3C" : step.label}
          </span>
        </li>
      ))}
    </ol>
...
export function SignupForm() {
    ...
    try {
      ...
      try {
        const codeResponse = await fetch("/api/portal/auth/team-code", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(
            invited ? { inviteToken: invite.token } : { code: teamCode },
          ),
        });
      ...
      setError(
        isExistingAccount ? ACCOUNT_EXISTS_ERROR : friendlyAuthError(err),
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <AuthShell>
      ...
      {/* An open invite is the only way in: its packet creates the account with
          the hire's role and checklist. A plain sign-up here would make a second,
          unlinked account and the invite link would then refuse them. */}
      {invited ? (
        <div className={a.stack}>
          <Link
            href={`/onboard/${invite.token}`}
            className={`${s.btnPrimary} ${a.btn}`}
          >
        ...
        <form onSubmit={handleSubmit} className={a.stack}>
          ...
          <div className={a.field}>
            ...
            <div className={a.strength}>
              <div
                className={a.strengthBar}
                data-level={password ? strength : undefined}
                aria-hidden="true"
              >
                <i />
                <i />
      ...
      <p className={a.note} role="note">
        Applied for a job? You don&apos;t need an account yet. We&apos;ll reach
        out after we review your application.
      </p>
    </AuthShell>
  );
}
