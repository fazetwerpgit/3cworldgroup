...
function QueueSection({
  ...
  return (
    <section className="portal-enter portal-enter-2 mt-[29px]">
      <div className="flex items-end justify-between gap-[15px] border-b border-[#8dc63f] pb-[11px]">
        <div>
          <h2 className="portal-display mt-0.5 text-[22px] font-black leading-none tracking-[-.01em] text-[#0A1F44] dark:text-[#f4f7fa]">
      ...
      {loading ? (
        ...
        items.map((item) => (
          <Link
            key={item.key}
            href={item.href}
            className={`group grid min-h-[78px] grid-cols-[28px_minmax(0,1fr)_17px] items-center gap-3 border-b border-[#0A1F44]/[.14] transition-colors duration-150 hover:bg-[#8dc63f]/[.07] dark:border-white/[.08] max-[700px]:grid-cols-[23px_minmax(0,1fr)_13px] max-[700px]:gap-[7px] ${item.admin ? 'border-l-2 border-l-[#8dc63f] bg-[#8dc63f]/[.045] pl-3' : ''}`}
          >
            ...
            <span className="line-copy min-w-0">
              ...
              <span className="mt-1 block truncate text-[9px] text-[#687384] dark:text-[#9caabd] max-[700px]:text-[8px]">
                {item.meta}
              </span>
            </span>
            <ChevronRight className="h-[15px] w-[15px] shrink-0 text-[#9caabd] transition-transform duration-150 group-hover:translate-x-0.5 max-[700px]:col-start-3 max-[700px]:row-start-1 max-[700px]:h-[13px] max-[700px]:w-[13px]" aria-hidden="true" />
          </Link>
        ))
      )}
    </section>
  ...
  return (
    <section className="portal-enter portal-enter-3 mt-[29px]" aria-label="Monthly leaderboard">
      {loading ? (
        ...
        <div className="portal-dashboard-empty-leaderboard">
...
export default function DashboardPage() {
  ...
  const { greeting, refreshedLabel } = mounted ? timeContext : initialTimeContext;
  ...
  const contextCopy = leadsWithQueue
    ? pendingSignups > 0
      ? `${pendingSignups} new signup${pendingSignups === 1 ? '' : 's'} need role assignment. ${pendingCount > 0 ? 'Clear the oldest approval, then make' : 'Make'} today's calls count.`
      : pendingCount > 0
        ? `${pendingCount} approval${pendingCount === 1 ? '' : 's'} are in flight. Clear the oldest, then make today's calls count.`
        : "Your queue is clear. Make today's calls count."
    : pendingCount > 0
      ? `${pendingCount} approval${pendingCount === 1 ? '' : 's'} are in flight. Start with the oldest, then make today's calls count.`
      : "Your numbers are clear. Make today's calls count.";
  ...
  const commandBlock = (
    <section className={`portal-enter ${leadsWithQueue ? 'portal-enter-3 border-[#8dc63f]/40' : 'portal-enter border-[#0A1F44]/[.14] dark:border-white/[.14]'} border-b pb-6`}>
      <PageTitle
        title={`${greeting}, ${firstName}`}
        meta={`${formatNumber(stats?.totalSales ?? 0)} sales this month`}
        actions={<Link href="/portal/calls" className="inline-flex min-h-[44px] items-center gap-2 rounded-full border border-[#8dc63f] px-3 py-[9px] font-sans text-[13px] font-bold text-[#5a8f1f] transition-colors duration-150 hover:bg-[#8dc63f] hover:text-[#0A1F44] dark:text-[#8dc63f] dark:hover:text-[#0A1F44]"><Phone className="h-4 w-4" aria-hidden="true" />Open today&apos;s calls</Link>}
      />
    ...
    <div className="relative -m-4 min-h-[calc(100vh-4rem)] overflow-hidden bg-[#f7f8f5] text-[#0A1F44] sm:-m-6 dark:bg-[#030916] dark:text-[#f4f7fa]">
      ...
      <div aria-label={`${roleLabel} dashboard`} className="relative z-10 mx-auto w-full max-w-[1180px] px-[clamp(14px,4vw,58px)] pb-[42px] pt-[19px] max-[430px]:px-3">
        <div className="flex flex-col">
          {commandBlock}
          <QueueSection items={queueItems} managerView={leadsWithQueue} loading={statsLoading} />
          <LeaderboardTicker entries={leaderboard} currentUser={currentUser} loading={leaderboardLoading} userId={user?.uid} />
        </div>
        <footer className="portal-enter portal-enter-4 mt-[25px] flex justify-between gap-3 border-t border-[#0A1F44]/[.14] pt-[13px] text-[9px] leading-[1.7] text-[#687384] dark:border-white/[.14] dark:text-[#9caabd] max-[700px]:block">
