...
import { LOG, Shell, TopBar } from '../Chrome';
import { money } from '../format';
import proof from '../proof.jpg';
import s from '../scoreboard.module.css';
import l from '../logsale.module.css';

export const metadata = { title: 'Log a sale · Scoreboard (C)' };
...
export default async function LogSalePage({ searchParams }: Props) {
  ...
  return (
    <Shell>
      <TopBar section="log" task={{ title: 'Log a sale' }} />
  ...
  return (
    <div className={l.defaultGrid}>
      <section className={`${s.panel} ${l.entry}`} aria-labelledby="scan-h">
        <span className={l.corners} aria-hidden />
...
function ScanView() {
  ...
  return (
    <div className={l.scanGrid}>
      ...
      <Link href={LOG} className={`${s.btnSecondary} ${l.cancel}`}>
        Cancel
      </Link>
    </div>
  );
...
function ReviewView() {
  ...
  return (
    <form className={l.reviewGrid} action={LOG}>
      ...
      <aside className={l.proofCard}>
        ...
        <Link href={LOG} className={`${s.btnSecondary} ${l.retake}`}>
