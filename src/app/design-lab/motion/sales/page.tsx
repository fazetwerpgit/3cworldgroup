...
// Temporary harness (untracked, never committed): Sales motion on mock data.
...
function Harness() {
  ...
  const noPlan = q.get('plan') === '0';
  const [pay, setPay] = useState(q.get('view') === 'pay');
  const [help, setHelp] = useState(false);
  const fiber = useMemo(
    () => ({ data: { scope: 'own' as const, lastReportAt: '2026-09-23T12:00:00Z', orders: ORDERS }, loading: false, error: null }),
    []
  );
  const bySale = useMemo(() => matchFiberOrdersToSales(SALES, ORDERS), []);
  const upcoming = useMemo(() => nextPayout(datedSales(SALES, bySale), noPlan ? null : rates, new Date(2026, 8, 23, 12)), [bySale, noPlan]);
  return (
